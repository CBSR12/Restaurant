...
app.post("/reservation", async (req, res) => {
  ...
  const { party_id, table_number, reservation_date, status, payment_method } = reservationValues;

  // Check if all required fields are provided
  if (!party_id || !table_number || !reservation_date || !status || !payment_method) {
    return res.status(400).send({ error: "All fields must be provided" });
  }

  const query = {
    text: `INSERT INTO Reservation (party_id, table_number, reservation_date, status, payment_method) 
           VALUES ($1, $2, $3, $4, $5) RETURNING *`,
    values: [party_id, table_number, reservation_date, status, payment_method], // Ensure party_id is included here
  };
